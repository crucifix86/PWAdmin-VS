...
namespace pwAdmin.Outro
{
    public class OctetsStream
    {
        ...
        public string Read2BytesString()
        {
            int len = ReadUInt16();
            byte[] buffer = ReadBytes(16);
            return Extensions.ByteArray_to_UnicodeString(buffer);
        }

        public string Read1BytesString()
        {
            byte len = ReadByte();
        ...
        public void WriteString2(string v)
        {
            ...
            buffer = Extensions.UnicodeString_to_ByteArray(v, len);
            WriteUInt32((uint)buffer.Length);
            WriteBytes(buffer);
        }
        ...
        public void WriteUString(string value)
        {
            ...
            WriteCUInt32((uint)bytes.Length);
            WriteBytes(bytes);
        }

        private byte[] WriteCUInt32(uint value)
        {
            if (value <= 0x7F)
            {
